...
class DelugeCollector(object):
    ...
    def collect(self):
        ...
        deluge_torrent_state_count = prometheus_client.core.GaugeMetricFamily(
            "deluge_torrent_state_count",
            "Number of torrents by state",
            labels=["state"],
        )
        ...
        deluge_torrent_label_count = prometheus_client.core.GaugeMetricFamily(
            "deluge_torrent_label_count",
            "Number of torrents by label",
            labels=["label"],
        )
    ...
    def get_connection(self):
        ...
        if not response_json["result"] or len(response_json["result"]) < 1:
            raise Exception(
                f"Get connection error! Bad response. Response: {response.text}"
            )
        server_id = response_json["result"][0][0]
        payload = {"method": "web.connect", "params": [server_id], "id": 1}
...
def main():
    ...
    try:
        ...
        logging.error(
            "Configuration error. The environment variable DELUGE_URL is mandatory"
        )
        sys.exit(1)
    ...
    try:
        ...
        logging.error(
            "Configuration error. The environment variable DELUGE_PASSWORD is mandatory"
        )
        sys.exit(1)
    ...
    logging.info(
        "Server listening in http://%s:%d/metrics", exporter_address, exporter_port
    )
    while True:
        time.sleep(1e9)
